...
function SquareButton(props: {
  ...
  hasAbuseReportAccess: boolean;
  baseUrl: string;
  locales: (typeof languageModuleMap)[ArrayElement<
    typeof SUPPORTED_COOKIE_LANGUAGES
  >]["next/event/$slug/detail"];
}) {
  const { alreadyReported, authenticated, hasAbuseReportAccess, locales } =
    props;
  ...
  const handleCopyToClipboard = async () => {
    await copyToClipboard(
      `${props.baseUrl}${location.pathname}${location.search}${location.hash}`
    );
    setHasCopied(true);
  };
  ...
  return (
    <OverlayMenu searchParam="event-overview-more" size="medium">
      ...
      {hasAbuseReportAccess && <OverlayMenu.ListItem></OverlayMenu.ListItem>}
    </OverlayMenu>
  );
}
