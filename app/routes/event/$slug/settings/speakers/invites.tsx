...
export async function loader(args: LoaderFunctionArgs) {
  ...
  const locales =
    languageModuleMap[language]["event/$slug/settings/speakers/invites"];

  const eventId = await getEventIdBySlug(params.slug);
  invariantResponse(eventId !== null, "Event not found", { status: 404 });

  const { submission, profiles } = await getInvitedProfilesToJoinEventAsSpeaker(
    {
      request,
      eventId,
      authClient,
  ...
  try {
    await revokeInviteOfProfileToJoinEventAsSpeaker({
      ...
      locales: {
        mail: {
          subject: {
            de: languageModuleMap.de["event/$slug/settings/speakers/invites"]
              .route.mail.cancelledInvitation.subject,
            en: languageModuleMap.en["event/$slug/settings/speakers/invites"]
              .route.mail.cancelledInvitation.subject,
          },
        },
      },
