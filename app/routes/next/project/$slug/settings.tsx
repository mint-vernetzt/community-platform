...
function ProjectSettings() {
  ...
  return (
    <>
      ...
      <div className="mv-w-full mv-min-h-screen	 mv-flex mv-flex-col mv-absolute mv-top-0 mv-bg-white">
        <div className="mv-flex mv-gap-2 mv-items-center mv-justify-between">
          <span className="mv-p-6">
            <h1 className="mv-text-2xl mv-m-0">Projekteinstellungen</h1>
          </span>
          <span className="mv-px-4">
            <svg
              width="32"
              height="33"
              viewBox="0 0 32 33"
        ...
        <ul>
          {navLinks.map((navLink) => {
            const classes = classNames(
              "mv-text-primary mv-text-lg mv-font-semibold",
            ...
            return (
              <li key={navLink.label} className="mv-px-6 mv-py-4">
                {/* TODO: H3 as h1 */}
                <Link to={navLink.to} className={classes} prefetch="intent">
                  {navLink.label}
                </Link>
              </li>
            );
