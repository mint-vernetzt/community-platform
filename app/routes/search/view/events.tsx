...
import type { MaybeEnhancedEvents } from "~/routes/explore/utils.server";
import { enhanceEventsWithParticipationStatus } from "~/routes/explore/utils.server";
import { getPublicURL } from "~/storage.server";
import { searchEventsViaLike } from "../utils.server";
...
export const loader = async ({ request, params }: LoaderArgs) => {
  ...
  // - Who has access?
  // - Do we need the separation betweem future and past events?

  return json(
    {
      events: enhancedEvents,
      // pastEvents: pastEvents, // - Do we need the separation betweem future and past events?
  ...
  return (
    <>
      {loaderData.events.length > 0 ? (
        <section
          // ref={futureRefCallback} // Pagination -> Do we want pagination with the latency?
      ...
      {/* {pastEvents.length > 0 ? (
            <section
              // ref={pastRefCallback} // TODO: Pagination -> Do we want pagination with the latency?
