...
async function main() {
  ...
  // If the main event has spots left, add participant automatically instead of putting him on the waiting list
  // In order add a participant from the waiting list, the user must first be added to all n-main events as participant
  // Problem: What if there are no spots left?

  // Option 1: If the user is not allowed to be on the waiting list (he is not yet participant of the main event) -> he has to move up in the main event waiting list
  ...
  // Checks if there are users that are on the waiting list of a sub-event
  ...
  console.log(
    `There are ${usersOnChildWaitingList.length} users on the waiting list of sub-events`
  );
  ...
  for (const waitingListEntry of usersOnChildWaitingList) {
    ...
    if (isUserParticipantOfParentEvent === false) {
      ...
      // 1. Can we add the user as participant to the main event (are there free spots left?)
      ...
      if (parentEvent === null) {
        console.error(
          `main event with id ${waitingListEntry.event.parentEventId} not found`
        );
        continue;
      }
      ...
      // Recursively check if the user can participate on main event -> collect how many (which persons on which event (Unique persons?))

      // can not participate -> collect how many (which persons on which event (Unique persons?))
    } else {
      // Is already participant
      // -> Other script already ensures that you are participant of all consecutive main events
    }
  }
}
