...
async function main() {
  try {
    await prismaClient.$queryRaw`
        ...
        returns trigger as $$
        begin
          insert into public.profiles (id, username, email, first_name, last_name, academic_title, terms_accepted, profile_visibility_id)
          values (new.id, new.raw_user_meta_data->>'username', new.email, new.raw_user_meta_data->>'firstName', new.raw_user_meta_data->>'lastName', new.raw_user_meta_data->>'academicTitle', Cast(new.raw_user_meta_data->>'termsAccepted' as Boolean), new.raw_user_meta_data->>'profileVisibilityId');
          return new;
        end;
